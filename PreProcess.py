...
def process_numeric(train, test, column_name):
    # get mean of training d ata
    numberOfInstances=train.shape[0]+test.shape[0]
    testRatio=test.shape[0]/numberOfInstances
    trainRatio=train.shape[0]/numberOfInstances

    mean = train[column_name].mean()*trainRatio+test[column_name].mean()*testRatio
